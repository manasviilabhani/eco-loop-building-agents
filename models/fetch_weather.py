...
def _fetch(lat: float, lon: float, start: str, end: str, tz: str, variables: list[str]) -> dict:
...
def _epw_header(loc: locations.Location, start: date, end: date, ground_temps: list[float]) -> list[str]:
    ...
    return [
        ...
        f"COMMENTS 1,Generated by models/fetch_weather.py from the Open-Meteo ERA5 archive. "
        f"Observed conditions for {loc.city} from {start} to {end}; NOT a typical meteorological year.",
...
def build_epw(loc: locations.Location) -> Path:
    ...
    print(f"  fetching observed weather {start} -> {end} for {loc.city}...")
    payload = _fetch(
        loc.latitude, loc.longitude, start.isoformat(), end.isoformat(), loc.tz_name, HOURLY_VARS
    )
    hourly = payload["hourly"]
    times = hourly["time"]
...
def main():
    ...
    if loc.weather_source != "observed":
        raise SystemExit(
            f"{loc.label} uses a bundled {loc.weather_source.upper()} weather file "
