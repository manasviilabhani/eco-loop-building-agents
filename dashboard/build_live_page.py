...
<style>
  /* Palette: a single warm neutral ramp, near-white through greige and brown
     to near-black. It is a *sequential* ramp, so the usual categorical checks
     (chroma floor, lightness band) fail it by design -- "reads gray" is the
     intent here, not a defect.

     What matters is that the two series stay tellable apart, and they do, by
     lightness rather than hue: baseline vs AI measures dE 34.8 under deutan,
     34.9 tritan, 35.0 normal (light) and 29.9/30.0/30.1 (dark) -- far above
     the 15 floor, and it survives greyscale print for the same reason.

     Baseline is the faint dashed line and AI the strong solid one, in both
     modes: identity is carried by weight and dash as well as value, so the
     pair never depends on colour alone. That doubles as the relief the light
     baseline (#a89f92, 2.44:1 on the surface) needs -- it is always directly
     labelled, in the legend, and in the table view.

     Dark mode is re-stepped from the same ramp against the dark surface and
     re-validated, not flipped. */
  :root {
    --surface: #f7f7f6;
    --panel: #ffffff;
    --text: #241d19;
    ...
    --weather: #6a513d;
    --good: #6a513d;
    --shadow: 0 1px 2px rgba(36,29,25,.05), 0 8px 24px rgba(36,29,25,.06);
  }
  @media (prefers-color-scheme: dark) {
    :root {
      --surface: #14100e; --panel: #241d19; --text: #f7f7f6; --text-2: #c3c3c2;
      --muted: #8b8073; --grid: #362b25; --hairline: #4a382c;
      --baseline: #8b8073; --ai: #e0e0df; --weather: #a89f92; --good: #a89f92;
      --shadow: 0 1px 2px rgba(0,0,0,.5), 0 8px 24px rgba(0,0,0,.4);
    }
  }
  :root[data-theme="light"] {
    --surface: #f7f7f6; --panel: #ffffff; --text: #241d19; --text-2: #6a513d;
    --muted: #8b8073; --grid: #e0e0df; --hairline: #c3c3c2;
    --baseline: #a89f92; --ai: #4a382c; --weather: #6a513d; --good: #6a513d;
    --shadow: 0 1px 2px rgba(36,29,25,.05), 0 8px 24px rgba(36,29,25,.06);
  }
  ...
  * { box-sizing: border-box; }
  ...
  /* A flat line at zero is a correct reading, not a broken page. This says so
     rather than leaving the viewer to guess. */
  .note { font-size: 13px; color: var(--text-2); background: var(--surface);
          border-left: 3px solid var(--muted); border-radius: 0 6px 6px 0;
          padding: 9px 12px; margin: 0 0 12px; max-width: 74ch; }
...
<div class="wrap">
  ...
  <header>
    <div>
      <h1>
        <svg class="logo" viewBox="0 0 36 36" role="img" aria-label="Eco-Loop">
          <path d="M30.22 13.55 A13 13 0 1 1 20.26 5.20" fill="none"
                stroke="var(--ai)" stroke-width="2.6" stroke-linecap="round"/>
          <path d="M24.20 5.90 L19.84 7.56 L20.68 2.84 Z" fill="var(--ai)"/>
          <path d="M10.8 20.4 L14.4 20.4 L18.1 25.4 L21.9 22.2 L25.4 23.6" fill="none"
                stroke="var(--baseline)" stroke-width="2.4"
                stroke-linecap="round" stroke-linejoin="round"/>
        </svg>Eco-Loop — live</h1>
  ...
  <div class="card">
    ...
    <p class="note" id="setback" hidden></p>
...
function drawChart(cv, cfg) {
  ...
  for (const s of cfg.series) {
    ...
    // Dash is a second, colour-independent channel of identity -- it is what
    // keeps baseline and AI apart in greyscale and for a reader who cannot
    // separate two steps of one warm ramp by value alone.
...
function attachHover(cv, tipEl, getCfg) {
  ...
  // Leaving the *document* outright -- which is what happens when the pointer
  // exits the component iframe this page is embedded in -- does not reliably
  // fire pointerleave on the canvas, so a tooltip could stay frozen on screen
  // after the cursor had gone. Catch that case too.
  for (const ev of ["pointerleave", "blur"]) {
    (ev === "blur" ? window : document).addEventListener(
      ev, () => (tipEl.style.opacity = 0));
  }
}
...
function render() {
  ...
  // Before the occupancy schedule starts, baseline and AI *are* the same
  // building: the HVAC sits in setback and the agent has no setpoint authority,
  // so the difference is exactly zero. That is a correct reading, but a flat
  // line at zero looks identical to a page that failed to load, so say which
  // one it is and when the curve will start moving.
  const startsAt = delta.findIndex((v) => Math.abs(v) >= 0.005);
  const setback = document.getElementById("setback");
  if (startsAt > 0 && t < startsAt) {
    setback.hidden = false;
    setback.textContent =
      `The building is unoccupied right now — the HVAC is in overnight setback, so the agent `
      + `has nothing to control and the difference is exactly zero. On this site's runs the `
      + `curve starts moving around ${pad2(startsAt)}:00 local, in `
      + `${Math.floor(startsAt - t)}h ${Math.round(((startsAt - t) % 1) * 60)}m.`;
  } else {
    setback.hidden = true;
  }
  ...
  cfgs.demand = {
    series: [
      { name: "Baseline", data: site.baselineKw, color: css("--baseline"),
        label: "Baseline", dash: [6, 4] },
      { name: "AI closed-loop", data: site.aiKw, color: css("--ai"),
        label: "AI", width: 2.4 },
    ],
